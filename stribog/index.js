...
const BLOCK_SIZE = 64; // длина блока
...
function E(k, m) {
    let state = m;
    for (let i = 0; i < 12; i++) {
        state = GOST_Hash_X(k, state);
        state = GOST_Hash_S(state);
        state = GOST_Hash_P(state);
        state = GOST_Hash_L(state);
        k = keySchedule(k, i);
    }
    state = GOST_Hash_X(k, state);
    return state;
...
function GN(n, h, m) {
    let k = GOST_Hash_X(h, n);
    k = GOST_Hash_S(k);
    k = GOST_Hash_P(k);
    k = GOST_Hash_L(k);
    let tmp = E(k, m);
    tmp = GOST_Hash_X(tmp, h);
    return GOST_Hash_X(tmp, m);
}
...
function hash(message) {
    ...
    let n = new Uint8Array(BLOCK_SIZE);
    let sigma = new Uint8Array(BLOCK_SIZE);

    let n512 = new Uint8Array(new Uint8Array([0, 2, 0, 0]), 0, BLOCK_SIZE); // byte representation of 512
    let inc = 0;
    while (len >= BLOCK_SIZE) {
        ...
        h = GN(n, h, tmpMsg);
        n = GOST_Hash_ADD512(n, n512).reverse();
        sigma = GOST_Hash_ADD512(sigma, tmpMsg);
        len -= BLOCK_SIZE;
    }

    let msg = message.slice(0, message.length - inc * 64);
    ...
    h = GN(n, h, paddedMsg);
    let msgLen = new Uint8Array(BLOCK_SIZE);
    for (let i = 0; i < 4; i++) {
        msgLen[i] = ([msg.length * 8] >> i * 8) & 255;
    }
    n = GOST_Hash_ADD512(n, msgLen.reverse());
    sigma = GOST_Hash_ADD512(sigma, paddedMsg);
...
// console.log(hash(new Uint8Array(encoder.encode('hello world'))).toString());
// console.log(GOST_Hash_ADD512(new Uint8Array('b1085bda1ecadae9ebcb2f81c0657c1f2f6a76432e45d016714eb88d7585c4fc4b7ce09192676901a2422a08a460d31505767436cc744d23dd806559f2a64507'.match(/.{1,2}/g).map(el => parseInt(el, 16))), new Uint8Array('b383fc2eced4a574b383fc2eced4a574b383fc2eced4a574b383fc2eced4a574b383fc2eced4a574b383fc2eced4a574b383fc2eced4a574b383fc2eced4a574'.match(/.{1,2}/g).map(el => parseInt(el, 16)))))
// console.log(hash(new Uint8Array(encoder.encode('hello world'))).toString())
console.log(hash(new Uint8Array('323130393837363534333231303938373635343332313039383736353433323130393837363534333231303938373635343332313039383736353433323130'.match(/.{1,2}/g).map(el => parseInt(el, 16)))).toString())
console.log(hash(new Uint8Array('fbe2e5f0eee3c820fbeafaebef20fffbf0e1e0f0f520e0ed20e8ece0ebe5f0f2f120fff0eeec20f120faf2fee5e2202ce8f6f3ede220e8e6eee1e8f0f2d1202ce8f0f2e5e220e5d1'.match(/.{1,2}/g).map(el => parseInt(el, 16)))).toString())